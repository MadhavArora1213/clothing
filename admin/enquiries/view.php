...
<div class="admin-content">
  <div class="admin-page-header">
    <div>
      <h1>Enquiry #<?= $id ?>: <?= esc($enquiry['subject']) ?></h1>
  ...
  <div class="admin-grid" style="display: grid; grid-template-columns: 2fr 1fr; gap: var(--space-4);">
    <div class="admin-card" style="padding: var(--space-6);">
      ...
      <div style="background: #f8fafc; padding: 16px; border-radius: 8px; border: 1px solid #e2e8f0; margin-bottom: 20px;">
        <p style="font-size: 14px; line-height: 1.6; color: #0f172a; white-space: pre-wrap; margin: 0;"><?= esc($enquiry['message']) ?></p>
      </div>

      <div style="font-size: 13px; color: var(--color-text-secondary); line-height: 1.6;">
        <p><strong>Sender Name:</strong> <?= esc($enquiry['name']) ?></p>
        <p><strong>Email Address:</strong> <a href="mailto:<?= esc($enquiry['email']) ?>"><?= esc($enquiry['email']) ?></a></p>
        <p><strong>Phone Number:</strong> <?= esc($enquiry['phone'] ?? 'Not provided') ?></p>
      </div>
    </div>
    ...
    <div class="admin-card" style="padding: var(--space-6);">
      ...
      <form method="POST" action="">
        ...
        <div class="form-group" style="margin-bottom: 16px;">
          ...
          <textarea name="reply" rows="4" placeholder="Notes about resolution or copy of email reply sent to customer..."><?= esc($enquiry['reply'] ?? '') ?></textarea>
        </div>
