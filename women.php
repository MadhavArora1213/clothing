...
$pageTitle       = 'Women — UOC | Premium Streetwear & Ethnic Fusion for Women';
$pageDescription = 'Shop women\'s kurtis, sarees, lehengas, western wear & ethnic fusion. Handcrafted in India. Free shipping above ₹999.';
$pageKeywords    = 'women streetwear india, kurti online, ethnic wear women, western tops women, UOC women';
$pageOgImage     = 'https://urbanoutfitshop.com/src/og-default.jpg';
$pageCanonical   = 'https://urbanoutfitshop.com/women.php';
$pageSchema      = '{
  "@type": "CollectionPage",
  "name": "Women — Urban Outfit Collection",
  "url": "https://urbanoutfitshop.com/women.php",
  "isPartOf": { "@id": "https://urbanoutfitshop.com/#website" }
}';
$currentPage = 'women';

// Fetch Women subcategories with image
$womenSubcats = [];
if ($mysqli) {
  $sc = $mysqli->query("
  ...
  ");
  if ($sc) $womenSubcats = $sc->fetch_all(MYSQLI_ASSOC);
}
...
if ($mysqli) {
  ...
  if (!empty($womenProducts)) {
    $ids = array_column($womenProducts, 'id');
    ...
    if ($ss) {
      ...
      foreach ($womenProducts as &$p) $p['sizes'] = $szMap[$p['id']] ?? [];
      unset($p);
    }
  }
}
...
<main class="uoc-main">
  ...
  <section class="dept-hero" id="deptHero">
    <div class="dept-hero-slider">
      <div class="dept-hero-slide active">
        <img src="<?= BASE_URL ?>/images/womenhero1.png" alt="Women's Collection" loading="eager">
      </div>
      <div class="dept-hero-slide">
        <img src="<?= BASE_URL ?>/images/womenhero2.png" alt="Women's Collection" loading="lazy">
      </div>
      <div class="dept-hero-slide">
        <img src="<?= BASE_URL ?>/images/womenhero3.png" alt="Women's Collection" loading="lazy">
      </div>
      <div class="dept-hero-slide">
        <img src="<?= BASE_URL ?>/images/womenhero4.png" alt="Women's Collection" loading="lazy">
      </div>
    </div>
  ...
  <div class="dept-hero-bottom">
    <div class="dept-hero-dots" id="heroDots">
      ...
      <span class="dept-hero-dot" onclick="goToSlide(2)"></span>
      <span class="dept-hero-dot" onclick="goToSlide(3)"></span>
    </div>
  </div>
  ...
  <section class="shop-cat-section">
    <div class="uoc-container">
      ...
      <div class="shop-cat-scroll" id="catScroll">
        <?php foreach ($womenSubcats as $sc): ?>
        <a href="<?= BASE_URL ?>/shop.php?category=women&subcategory=<?= htmlspecialchars($sc['slug']) ?>" class="shop-cat-item">
          ...
          <?php else: ?>
          <img src="https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?w=400&h=500&auto=format&fit=crop&q=80" alt="<?= htmlspecialchars($sc['name']) ?>" class="shop-cat-item-img" loading="lazy">
          <?php endif; ?>
        </a>
        <?php endforeach; ?>
      </div>
    ...
    <div class="uoc-container">
      <div class="uoc-section-header">
        <div>
          ...
          <h2 class="uoc-section-title">Trending In Women</h2>
        </div>
        <a href="<?= BASE_URL ?>/shop.php?category=women" class="uoc-view-all">View All &rarr;</a>
      </div>

      <div class="uoc-product-grid">
        <?php foreach ($womenProducts as $i => $item): ?>
        <div class="uoc-product-card">
          ...
          <div class="uoc-product-actions">
            <button class="uoc-btn-add-cart" onclick="deptAddToCart(<?= $item['id'] ?>, <?= json_encode($item['sizes'] ?? []) ?>)" title="Add to Cart">
            ...
            <button class="uoc-btn-buy-now" onclick="deptBuyNow(<?= $item['id'] ?>, <?= json_encode($item['sizes'] ?? []) ?>)" title="Buy Now">
              Buy Now
            </button>
          </div>
        </div>
...
function deptAddToCart(productId, sizes) {
...
function deptBuyNow(productId, sizes) {
  var size = (sizes && sizes.length > 0) ? sizes[0] : '';
  fetch('<?= BASE_URL ?>/api/cart.php', {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
